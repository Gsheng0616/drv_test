...
static ssize_t spi_drv_write(struct file *file, const char __user *buf, size_t size, loff_t *offset)
{
	...
	unsigned char kerel_buf[2];


	/*从用户空间拷贝数据*/
	err = copy_from_user(&value, buf, size);
	...
	xfer[0].tx_buf = kerel_buf;
	xfer[0].len = 2;
	
	/*通过spi控制器传输数据*/
    err = spi_sync_transfer(g_spi_device, xfer, 1);
...
static int spi_drv_probe(struct spi_device *spi)
{
	...
	device_create(spi_class, NULL, MKDEV(major, 0), NULL, "mytlc5615"); /* /dev/myspi */
	return 0;
}
